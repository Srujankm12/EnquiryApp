...
const MyRFQsScreen: React.FC = () => {
  ...
  const handleToggleStatus = async (rfqId: string, currentActive: boolean) => {
    try {
      ...
      setRfqs((prev) =>
        prev.map((r) => (r.id === rfqId ? { ...r, is_rfq_active: !currentActive } : r))
      );
    } catch (error: any) {
      ...
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            const token = await AsyncStorage.getItem('token');
            const headers = { Authorization: `Bearer ${token}` };
            await axios.delete(`${API_URL}/rfq/delete`, {
              headers,
              data: { rfq_id: rfqId },
            });
    ...
    <View key={`${rfq.id}-${index}`} style={styles.rfqCard}>
      ...
      <View style={styles.cardHeader}>
        ...
        <View style={[styles.statusBadge, rfq.is_rfq_active ? styles.activeBadge : styles.inactiveBadge]}>
          <Text style={[styles.statusBadgeText, rfq.is_rfq_active ? styles.activeText : styles.inactiveText]}>
      ...
      <View style={styles.detailsContainer}>
        ...
        {rfq.city ? (
          <View style={styles.detailItem}>
            ...
            <Text style={styles.detailText}>{rfq.city}{rfq.state ? `, ${rfq.state}` : ''}</Text>
          </View>
        ) : null}
      </View>
      ...
      <View style={styles.cardFooter}>
        ...
        >
          <Ionicons
            name={rfq.is_rfq_active ? 'pause-circle-outline' : 'play-circle-outline'}
            size={16}
            color={rfq.is_rfq_active ? '#FF9500' : '#34C759'}
          />
          <Text style={[styles.actionBtnText, { color: rfq.is_rfq_active ? '#FF9500' : '#34C759' }]}>
        ...
        >
          ...
          <Text style={[styles.actionBtnText, { color: '#DC3545' }]}>Delete</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
